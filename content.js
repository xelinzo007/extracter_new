...
if (window.__contentScriptLoaded) {
  ...
  async function loadConfig() {
    ...
    try {
      ...
      const response = await originalFetch(configUrl);
  ...
  // Dynamic data storage for anti-blocking
  const dynamicData = {
    successfulRequests: [],
    capturedHeaders: new Map(),
    capturedCookies: new Map(),
    userInfoPatterns: [],
    headerPatterns: {},
    cookiePatterns: {},
    fingerprint: null,
  ...
  /**
   * Generate dynamic browser fingerprint
   * @returns {Object} - Browser fingerprint object
   */
  function generateDynamicFingerprint() {
    if (dynamicData.fingerprint) {
      return dynamicData.fingerprint;
    }
    ...
    ctx.font = '14px Arial';
    ctx.fillText('Dynamic fingerprint', 2, 2);
    ...
    dynamicData.fingerprint = fingerprint;
    return fingerprint;
  }

  /**
   * Extract userInfo from various sources on the page dynamically
   * @returns {Object|null} - Extracted userInfo object or null
   */
  function extractUserInfoFromPage() {
    try {
      // Try to extract from common window objects
      const sources = [
        () => window.userInfo,
        () => window.__userInfo,
        () => window.mmtUserInfo,
        () => window.makemytripUserInfo,
        () => window.appState?.userInfo,
        () => window.appState?.user?.info,
        () => window.reduxStore?.getState?.()?.user?.info,
        () => window.__INITIAL_STATE__?.userInfo,
        () => window.__APP_STATE__?.userInfo,
        () => window.__NEXT_DATA__?.props?.pageProps?.userInfo,
        () => window.__REACT_QUERY_STATE__?.queries?.find(q => q?.queryKey?.includes('userInfo'))?.state?.data,
        () => {
          try {
            const val = window.localStorage?.getItem('userInfo');
            return val ? safeJsonParse(val, null) : null;
          } catch (e) {
            return null;
          }
        },
      ...
      for (const getSource of sources) {
        try {
          const userInfo = getSource();
          if (userInfo && typeof userInfo === 'object') {
            console.log("[Content Script] ✅ Extracted userInfo from page:", userInfo);
            // Store pattern for future use
            dynamicData.userInfoPatterns.push(userInfo);
            return userInfo;
          }
        } catch (e) {
      ...
      // Try to extract from API responses stored in window
      try {
        if (window.__API_RESPONSES__) {
          for (const response of Object.values(window.__API_RESPONSES__)) {
            if (response?.data?.userInfo) {
              console.log("[Content Script] ✅ Found userInfo in API responses");
              dynamicData.userInfoPatterns.push(response.data.userInfo);
      ...
      // Try to extract from successful network requests
      if (dynamicData.successfulRequests.length > 0) {
        const lastSuccess = dynamicData.successfulRequests[dynamicData.successfulRequests.length - 1];
        if (lastSuccess?.responseData) {
          try {
            const parsed = typeof lastSuccess.responseData === 'string' 
              ? JSON.parse(lastSuccess.responseData) 
              : lastSuccess.responseData;
            if (parsed?.userInfo || parsed?.data?.userInfo) {
              const extracted = parsed.userInfo || parsed.data.userInfo;
              console.log("[Content Script] ✅ Extracted userInfo from successful API response");
              dynamicData.userInfoPatterns.push(extracted);
              return extracted;
            }
          } catch (e) {
            // Try to find userInfo in response text
            const text = lastSuccess.responseData.toString();
            const userInfoMatch = text.match(/"userInfo"\s*:\s*({[^}]+})/);
            if (userInfoMatch) {
              try {
                const extracted = JSON.parse(userInfoMatch[1]);
                dynamicData.userInfoPatterns.push(extracted);
                return extracted;
              } catch (e) {}
            }
      ...
      console.warn("[Content Script] Could not extract userInfo from page:", error);
      return null;
    }
  }

  /**
   * Capture and learn from successful API requests
   * @param {Object} requestData - Request data including headers, cookies, response
   */
  function learnFromSuccessfulRequest(requestData) {
    try {
      if (!requestData || !requestData.url) return;

      // Store successful request
      dynamicData.successfulRequests.push({
        ...requestData,
        timestamp: Date.now()
      });

      // Keep only last 50 successful requests
      if (dynamicData.successfulRequests.length > 50) {
        dynamicData.successfulRequests.shift();
      }

      // Learn header patterns
      if (requestData.headers) {
        Object.entries(requestData.headers).forEach(([key, value]) => {
          const lowerKey = key.toLowerCase();
          if (!dynamicData.headerPatterns[lowerKey]) {
            dynamicData.headerPatterns[lowerKey] = [];
          }
          dynamicData.headerPatterns[lowerKey].push(value);
          // Keep only last 10 values per header
          if (dynamicData.headerPatterns[lowerKey].length > 10) {
            dynamicData.headerPatterns[lowerKey].shift();
          }
        });
      }

      // Learn cookie patterns
      if (requestData.cookies) {
        const cookies = typeof requestData.cookies === 'string' 
          ? requestData.cookies.split(';').map(c => c.trim())
          : Object.entries(requestData.cookies);
        
        cookies.forEach(cookie => {
          const [name, value] = typeof cookie === 'string' 
            ? cookie.split('=').map(c => c.trim())
            : cookie;
          if (name && value) {
            if (!dynamicData.cookiePatterns[name]) {
              dynamicData.cookiePatterns[name] = [];
            }
            dynamicData.cookiePatterns[name].push(value);
            if (dynamicData.cookiePatterns[name].length > 10) {
              dynamicData.cookiePatterns[name].shift();
            }
          }
        });
      }
      ...
      dynamicData.lastSuccessfulRequest = {
        ...requestData,
        timestamp: Date.now()
      };


      console.log("[Content Script] ✅ Learned from successful request");
    } catch (error) {
      console.warn("[Content Script] Error learning from successful request:", error);
    }
  }

  /**
   * Get learned headers for a request
   * @param {string} url - Request URL
   * @returns {Object} - Headers object
   */
  function getLearnedHeaders(url) {
    const headers = {};

    // Use headers from last successful request if available
    if (dynamicData.lastSuccessfulRequest?.headers) {
      Object.assign(headers, dynamicData.lastSuccessfulRequest.headers);
    }

    // Apply learned patterns
    Object.entries(dynamicData.headerPatterns).forEach(([key, values]) => {
      if (values.length > 0) {
  ...
  /**
   * Get learned cookies for a request
   * @param {string} url - Request URL
   * @returns {string} - Cookie string
   */
  function getLearnedCookies(url) {
    const cookies = [];

    // Use cookies from last successful request if available
    if (dynamicData.lastSuccessfulRequest?.cookies) {
      if (typeof dynamicData.lastSuccessfulRequest.cookies === 'string') {
        cookies.push(dynamicData.lastSuccessfulRequest.cookies);
      } else {
        Object.entries(dynamicData.lastSuccessfulRequest.cookies).forEach(([name, value]) => {
    ...
    // Apply learned patterns
    Object.entries(dynamicData.cookiePatterns).forEach(([name, values]) => {
      if (values.length > 0) {
        cookies.push(`${name}=${values[values.length - 1]}`);
      }
    });
  ...
  /**
   * Safe JSON parse with validation and error handling
  ...
  function safeJsonParse(jsonString, defaultValue = null) {
    ...
    // Check if it looks like binary/corrupted data
    const firstChar = jsonString.charCodeAt(0);
    // If first character is not a valid JSON start character ({, [, ", number, or whitespace)
    if (firstChar !== 0x7B && // {
        firstChar !== 0x5B && // [
        firstChar !== 0x22 && // "
        firstChar !== 0x2D && // -
        (firstChar < 0x30 || firstChar > 0x39) && // not 0-9
        firstChar !== 0x74 && // t (true)
        firstChar !== 0x66 && // f (false)
        firstChar !== 0x6E && // n (null)
        firstChar > 32) { // not whitespace
      console.warn("[Content Script] ⚠️ Invalid JSON detected (binary/corrupted data):", jsonString.substring(0, 50));
      return defaultValue;
    }
    ...
    if (!trimmed.match(/^[\s]*[{\[\"]/)) {
      console.warn("[Content Script] ⚠️ JSON doesn't start with valid character:", trimmed.substring(0, 50));
      return defaultValue;
    }
    ...
    try {
      ...
      console.warn("[Content Script] ⚠️ JSON parse error:", e.message, "Data preview:", trimmed.substring(0, 100));
      return defaultValue;
    }
  }
  ...
  const storageResults = new Map(); // Track storage results: URL -> { success: boolean, timestamp: number }
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (response && response.success) {
            ...
            storageResults.set(urlString, { success: true, timestamp: Date.now() });
          } else if (response?.skipped) {
            console.log(
              "[Content Script] ℹ️ Insert API skipped:",
              response.reason,
            );
            // Track skipped storage
            storageResults.set(urlString, { success: false, skipped: true, timestamp: Date.now() });
            ...
            storageResults.set(urlString, { success: false, error: response.error, timestamp: Date.now() });
          } else {
            // Track unknown result
            storageResults.set(urlString, { success: false, error: "Unknown error", timestamp: Date.now() });
          }
        },
      );
    } catch (error) {
  ...
  (function () {
    ...
    window.__originalFetch = originalFetch; // Store for loadConfig to use
    ...
    window.fetch = async function (...args) {
      try {
        ...
        // Skip interception for config.json to avoid infinite loop
        if (urlString.includes("config.json") || urlString.includes("chrome-extension://")) {
        ...
        // Log fetch requests (excluding config.json which is already skipped)
        console.log("[Content Script] 🔍 Fetch request:", method, urlString);

        // Check API names synchronously using cached config or fallback
        // This prevents race conditions from async config loading
        let isSearchStreamAPI = false;
        let isAPI = false;
        
        try {
          // Try to use cached config first (synchronous check)
          if (config && config.api_names) {
            isSearchStreamAPI =
              urlString.includes(config.api_names.searchStreamAPI) ||
              urlString.includes(config.api_names.searchStreamAPIFull);
            isAPI =
              urlString.includes(config.api_names.apiPath) ||
              urlString.includes(
                (config.domain || "").replace("https://", "").replace("http://", "") +
                  config.api_names.apiPath,
              );
          } else {
            // Fallback to pattern matching if config not loaded yet
            isSearchStreamAPI =
          ...
          // If any error, use fallback patterns
          console.warn("[Content Script] Error checking API names, using fallback:", e);
          isSearchStreamAPI =
            urlString.includes("/search-stream-dt") ||
            urlString.includes("search-stream-dt");
          isAPI =
        ...
        // Load config asynchronously for future use (non-blocking)
        // Only load if not already loaded to avoid repeated calls
        if (!config) {
          loadConfig().catch(() => {
            // Silently fail - we already have fallback patterns
          });
        }
      ...
      if (isSearchStreamAPI) {
        ...
        // Apply learned headers from successful requests
        let learnedHeaders = {};
        let learnedCookies = null;
        let randomUA = navigator.userAgent; // Fallback to default user agent
        
        try {
          learnedHeaders = getLearnedHeaders(urlString) || {};
        } catch (e) {
          console.warn("[Content Script] Error getting learned headers:", e);
        }
        
        try {
          learnedCookies = getLearnedCookies(urlString);
        } catch (e) {
          console.warn("[Content Script] Error getting learned cookies:", e);
        }
        ...
        try {
          ...
          console.warn("[Content Script] Error getting random user agent:", e);
          randomUA = navigator.userAgent; // Fallback
        }

        // Modify options to include learned headers and random user agent
        const modifiedOptions = { ...options };
        if (!modifiedOptions.headers) {
          modifiedOptions.headers = {};
        }

        // Merge learned headers
        Object.entries(learnedHeaders).forEach(([key, value]) => {
          if (key.toLowerCase() !== 'user-agent') { // Don't override user-agent yet
            if (modifiedOptions.headers instanceof Headers) {
              modifiedOptions.headers.set(key, value);
            } else {
              modifiedOptions.headers[key] = value;
            }
            requestHeaders[key] = value;
          }
        });

        // Add cookies if learned
        if (learnedCookies) {
          if (modifiedOptions.headers instanceof Headers) {
            modifiedOptions.headers.set("Cookie", learnedCookies);
          } else {
            modifiedOptions.headers["Cookie"] = learnedCookies;
            modifiedOptions.headers["cookie"] = learnedCookies;
          }
          requestHeaders["Cookie"] = learnedCookies;
          requestHeaders["cookie"] = learnedCookies;
        }
        ...
        // Add dynamic timing - random delay to mimic human behavior
        const timingDelay = Math.floor(Math.random() * 200) + 50; // 50-250ms random delay
        dynamicData.requestTimings.push({
          timestamp: Date.now(),
          delay: timingDelay
        });
        // Keep only last 20 timings
        if (dynamicData.requestTimings.length > 20) {
          dynamicData.requestTimings.shift();
        }
      ...
      if (isSearchStreamAPI) {
        ...
        (async () => {
          try {
            ...
            // Learn from successful requests (status 200 and has data)
            if (response.ok && response.status === 200 && responseData && responseData.length > 0) {
              // Get cookies from document or headers
              const cookies = document.cookie || '';
              learnFromSuccessfulRequest({
                url: urlString,
                method: method,
                headers: responseInfo.requestHeaders,
                cookies: cookies,
                responseData: responseData,
                status: response.status,
                responseHeaders: responseInfo.responseHeaders
              });
              console.log("[Content Script] ✅ Learned from successful API request");
            }
        ...
        // If any error occurs in our interception code, log it but don't break the page
        // Fall back to original fetch to ensure page continues to work
        console.error("[Content Script] Error in fetch interception:", error);
        try {
          // Try to call original fetch as fallback
          return await originalFetch.apply(this, args);
        } catch (fallbackError) {
          // If even original fetch fails, re-throw the original error
          throw fallbackError;
        }
      }
    };
    ...
    XMLHttpRequest.prototype.open = function (method, url, ...rest) {
      try {
        ...
        // If any error occurs, fall back to original open
        console.error("[Content Script] Error in XMLHttpRequest.open interception:", error);
        return originalXHROpen.apply(this, [method, url, ...rest]);
      }
    };
    ...
    XMLHttpRequest.prototype.send = function (...args) {
      try {
        ...
        try {
          ...
          if (config && config.api_names) {
            isAPI =
              url &&
              (url.includes(config.api_names.apiPath) ||
                url.includes(
                  (config.domain || "").replace("https://", "").replace("http://", "") +
                    config.api_names.apiPath,
                ));
          ...
          // If any error, use fallback patterns
          console.warn("[Content Script] Error checking XHR API names, using fallback:", e);
          isAPI =
            url &&
            (url.includes("/api/") || url.includes("makemytrip.com/api"));
          isSearchStreamAPI =
      ...
      if (isSearchStreamAPI) {
        ...
        // Add random user agent for flight API calls
        let randomUA = navigator.userAgent; // Fallback
        try {
          randomUA = getRandomUserAgent();
        } catch (e) {
          console.warn("[Content Script] Error getting random user agent for XHR:", e);
        }
        
        try {
          this.setRequestHeader("User-Agent", randomUA);
        } catch (e) {
          console.warn("[Content Script] Error setting User-Agent header:", e);
        }
        console.log(
      ...
      this.addEventListener = function (type, listener, ...rest) {
        ...
        // If any error occurs in our interception code, log it but don't break the page
        // Fall back to original send to ensure page continues to work
        console.error("[Content Script] Error in XMLHttpRequest.send interception:", error);
        return originalXHRSend.apply(this, args);
      }
    };
      ...
      (async () => {
        ...
        while (attempts < maxAttempts) {
          ...
          if (streamAPIsWithData.length > 0 || attempts >= maxAttempts - 1) {
            ...
            if (downloadCount > 0) {
              // Wait a bit for storage to complete, then verify
              await new Promise((resolve) => setTimeout(resolve, 2000)); // Wait 2 seconds for storage
              
              // Verify storage results for all processed URLs
              let storageVerified = true;
              let storageErrors = [];
              streamAPIs.forEach((apiCall) => {
                if (apiCall.responseData || apiCall.rawStream || apiCall.events) {
                  const storageResult = storageResults.get(apiCall.url);
                  if (!storageResult || !storageResult.success) {
                    storageVerified = false;
                    storageErrors.push({
                      url: apiCall.url,
                      result: storageResult || { error: "No storage result found" }
                    });
                  }
                }
              });
              ...
              sendResponse({
                ...
                message: `Found ${streamAPIs.length} /search-stream-dt API call(s). Downloading ${downloadCount}...${storageVerified ? " Data storage verified." : " ⚠️ Storage verification failed for some URLs."}`,
              });
            } else {
