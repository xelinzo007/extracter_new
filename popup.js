...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const isScheduled = urlParams.get('scheduled') === '1';
  ...
  // If opened from scheduled task, check if processing is already running and show status
  if (isScheduled && isStandalone) {
    setTimeout(async () => {
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (response && response.isProcessing) {
            // Processing is already running, just show the UI
            updateStatus('Processing URLs from scheduled task...', 'loading');
            showProgress();
            startElapsedTimer();
            if (processUrlsBtn) {
            ...
            // Processing not running, try to start it
            startProcessing();
          }
        }
      );
