 ...
 */
async function closeChromeWindowsSafely() {
  try {
    console.log("[Background] 🚪 Closing Chrome windows (excluding extension popup)...");
    ...
    for (const window of windows) {
      if (window.id !== persistentWindowId) {
        try {
          ...
          console.warn(`[Background] Could not close window ${window.id}:`, error);
        }
      }
    }
    console.log(`[Background] ✅ Closed ${closedCount} Chrome window(s) (extension popup preserved)`);
    return true;
  } catch (error) {
    console.error("[Background] ❌ Error closing browser windows:", error);
    return false;
  }
}

/**
 * Helper function to safely clear browser data
 */
async function clearBrowserDataSafely() {
  try {
    console.log("[Background] 🧹 Clearing all browser data including history...");
      ...
      chrome.browsingData.remove(
        ...
        () => {
          if (chrome.runtime.lastError) {
            ...
            console.log("[Background] ✅ All browser data (including history) cleared successfully");
            resolve();
          }
        },
      );
    ...
    console.error("[Background] ❌ Error clearing browser data:", error);
    return false;
  }
}
...
async function storeResponseInCollection({
  ...
  while (retryCount <= MAX_RETRIES) {
    ...
    // NOTE: We only use this now for logging; it no longer changes control-flow.
    const responseLower = responseString.toLowerCase();
    const contains200OK =
      /\b200\s*ok\b/i.test(responseString) ||
    ...
    if (!response.ok) {
      ...
      if (contains200OK) {
        console.warn(
          "[Background] ⚠️ Response body contains '200 OK' text, treating as normal success and continuing.",
        );
      }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "openTab") {
    ...
    return true; // Keep channel open for async response
  } else if (request.action === "checkProcessingStatus") {
    // Check if processing is currently running
    sendResponse({
      isProcessing: isProcessingUrls,
      isScheduledRunning: isScheduledExecutionRunning,
    });
    return true;
...
async function openUrlsFromConfig() {
  try {
    ...
    // Load config to allow user-controlled parallelism and pacing
    const cfg = await loadConfig();

    // Rate-limit-aware parameters for opening flight search URLs
    // TARGET_TOTAL_DURATION_MS: how long (in total) we roughly want to spread all URLs.
    // MAX_PARALLEL_TABS: how many tabs can be processing in parallel.
    const rawMaxParallel =
      cfg && typeof cfg.maxParallelTabs !== "undefined"
        ? Number(cfg.maxParallelTabs)
        : NaN;
    const rawTargetMinutes =
      cfg && typeof cfg.targetDurationMinutes !== "undefined"
        ? Number(cfg.targetDurationMinutes)
        : NaN;

    // Defaults: ~4.25 hours total, 2 tabs in parallel
    const MAX_PARALLEL_TABS =
      Number.isFinite(rawMaxParallel) && rawMaxParallel > 0
        ? Math.min(Math.floor(rawMaxParallel), 6) // hard cap to avoid extreme values
        : 2;
    ...
    const TARGET_TOTAL_DURATION_MS = targetMinutes * 60 * 1000;

    // Compute a base delay so that, in expectation, all URLs are started within the target window.
    // Formula (approx): baseDelay ≈ (targetDuration * concurrency) / totalUrls
    const estimatedBaseDelay =
      totalUrls > 0
        ? Math.floor(
            (TARGET_TOTAL_DURATION_MS * MAX_PARALLEL_TABS) / totalUrls,
          )
        : 60000;

    // Clamp base delay to a reasonable range (at least 10s, at most 5 minutes)
    const BASE_DELAY_BETWEEN_TABS = Math.min(
      Math.max(estimatedBaseDelay, 10_000),
      5 * 60 * 1000,
    );

    // Jitter is 50% of base delay, capped at 60 seconds, to avoid perfectly regular patterns
    const JITTER_MS = Math.min(Math.floor(BASE_DELAY_BETWEEN_TABS / 2), 60_000);

    /**
     * Helper to wait with base delay + random jitter
     */
    const waitWithJitter = async (baseMs) => {
      const jitter = Math.floor(Math.random() * JITTER_MS);
      const delay = baseMs + jitter;
      console.log(
        `[Background] ⏳ Rate-limit wait: ${delay}ms (base: ${baseMs}ms, jitter: ${jitter}ms)`,
      );
      await new Promise((resolve) => setTimeout(resolve, delay));
    };

    /**
     * Open a single URL in a new tab, inject content script when loading starts.
     * Kept very close to previous behavior, but factored out for queue control.
     */
    const openSingleUrl = async (url, index) => {
      try {
        ...
        tabIds.push(tab.id);
        console.log(
          `[Background] Opened tab ${index + 1}/${totalUrls}:`,
          tab.id,
          url,
        );
        ...
        console.error(`[Background] Error opening URL ${index + 1}:`, error);
        // Continue with next URL
      }
    };

    /**
     * Process URLs with simple concurrency cap and spaced starts to reduce rate limiting.
     */
    let active = 0;
    let currentIndex = 0;
    ...
    await new Promise((resolve) => {
      const launchNext = async () => {
        // Launch new tasks while under concurrency cap
        while (active < MAX_PARALLEL_TABS && currentIndex < totalUrls) {
          const idx = currentIndex++;
          const url = urls[idx];
          active++;

          // Stagger new tab creation with delay + jitter to avoid bursts
          (async () => {
            if (idx > 0) {
              await waitWithJitter(BASE_DELAY_BETWEEN_TABS);
            }
            await openSingleUrl(url, idx);
            active--;
            if (currentIndex >= totalUrls && active === 0) {
              resolve();
            } else {
              // Try to launch more once a slot frees
              launchNext();
            }
          })();
        }

        // If no more URLs and no active tasks, resolve
        if (currentIndex >= totalUrls && active === 0) {
          resolve();
        }
      };
  ...
  for (let idx = 0; idx < failedUrls.length; idx++) {
    ...
    chrome.runtime
      .sendMessage({
        ...
        completed: totalProcessedSoFar,
        total: totalProcessedSoFar + failedUrls.length, // Include retries in total
        message: `🔄 Retrying failed URL ${idx + 1}/${failedUrls.length} (Attempt 1/3)...`,
        alert: true,
        alertType: "info",
      })
      .catch(() => {});
    ...
    while (retryAttempt < MAX_RETRIES && !retrySuccess) {
      retryAttempt++; // Increment at start (attempt 1, 2, 3)
      
      // Update progress for retry attempt
      chrome.runtime
        .sendMessage({
          action: "urlProcessingProgress",
          completed: totalProcessedSoFar + idx,
          total: totalProcessedSoFar + failedUrls.length,
          message: `🔄 Retrying failed URL ${idx + 1}/${failedUrls.length} (Attempt ${retryAttempt}/${MAX_RETRIES})...`,
        })
        .catch(() => {});
      try {
        // Open tab
        ...
        if (downloadResponse && downloadResponse.success) {
          console.log(
            `[Background] ✅ Retry successful for URL ${idx + 1}/${failedUrls.length} (Attempt ${retryAttempt}/${MAX_RETRIES})`,
          );
          retryResults.push({
            url: url,
            success: true,
          ...
          chrome.runtime
            .sendMessage({
              action: "urlProcessingProgress",
              completed: totalProcessedSoFar + idx + 1,
              total: totalProcessedSoFar + failedUrls.length,
              message: `✅ Retry successful for URL ${idx + 1}/${failedUrls.length} (Attempt ${retryAttempt}/${MAX_RETRIES})`,
            })
            .catch(() => {});
        } else {
          console.log(
            `[Background] ⚠️ Retry attempt ${retryAttempt}/${MAX_RETRIES} failed for URL ${idx + 1}/${failedUrls.length}.`,
          );
          
          // If we have more attempts, wait before next retry
          if (retryAttempt < MAX_RETRIES) {
            ...
            chrome.runtime
              .sendMessage({
                ...
                completed: totalProcessedSoFar + idx,
                total: totalProcessedSoFar + failedUrls.length,
                message: `⏳ Retry attempt ${retryAttempt}/${MAX_RETRIES} failed. Waiting 5 minutes before attempt ${retryAttempt + 1}/${MAX_RETRIES}...`,
                alert: true,
                alertType: "warning",
              })
              .catch(() => {});
        ...
        console.error(
          `[Background] Error in retry attempt ${retryAttempt}/${MAX_RETRIES} for URL ${idx + 1}:`,
          error,
        );
        ...
        if (retryAttempt < MAX_RETRIES) {
          ...
          chrome.runtime
            .sendMessage({
              ...
              completed: totalProcessedSoFar + idx,
              total: totalProcessedSoFar + failedUrls.length,
              message: `⏳ Error on retry attempt ${retryAttempt}/${MAX_RETRIES}. Waiting 5 minutes before attempt ${retryAttempt + 1}/${MAX_RETRIES}...`,
              alert: true,
              alertType: "warning",
            })
            .catch(() => {});
    ...
    if (!retrySuccess) {
      console.warn(
        `[Background] ❌ All ${MAX_RETRIES} retry attempts failed for URL ${idx + 1}/${failedUrls.length}`,
      );
      retryResults.push({
        url: url,
        success: false,
        ...
        error: `All ${MAX_RETRIES} retry attempts failed`,
      });
      
      // Update progress on final failure
      chrome.runtime
        .sendMessage({
          action: "urlProcessingProgress",
          completed: totalProcessedSoFar + idx + 1,
          total: totalProcessedSoFar + failedUrls.length,
          message: `❌ All ${MAX_RETRIES} retry attempts failed for URL ${idx + 1}/${failedUrls.length}`,
          alert: true,
          alertType: "error",
        })
        .catch(() => {});
...
async function processUrlsSequentially() {
  ...
  try {
    ...
    let totalUrlsSeen = 0; // Track total URLs we've seen (for progress tracking)
    ...
    while (true) {
      ...
      const totalUrls = urls.length;
      totalUrlsSeen += totalUrls; // Track total URLs we've seen
      console.log(
        `[Background] Processing batch: ${totalUrls} URLs (Total processed so far: ${totalProcessed}, Total seen: ${totalUrlsSeen})`,
      );

      const processedUrls = [];
      let batchCompletedEarly = false; // Track if we broke early after 40 URLs

      // Send initial progress - use totalUrlsSeen as estimated total
      chrome.runtime
        .sendMessage({
          action: "urlProcessingProgress",
          completed: totalProcessed,
          total: totalUrlsSeen, // Use total URLs seen as estimated total
          message: `Processing batch: ${totalUrls} URLs (Total processed: ${totalProcessed})...`,
        })
        .catch(() => {});
      ...
      for (let i = 0; i < totalUrls; i++) {
        ...
        try {
          ...
          while (urlRetryCount < MAX_RETRIES_FOR_URL && !urlProcessed) {
            try {
              ...
              if (!currentTab || !currentTab.id) {
                console.warn(`[Background] ⚠️ Current tab is invalid for URL ${i + 1}. Skipping...`);
                processedUrls.push({ 
                  url: url, 
                  success: false, 
                  tabId: null, 
                  error: "Tab invalid or closed",
                  skipped: true
                });
                urlProcessed = true;
                break;
              ...
              if (downloadResponse && downloadResponse.success) {
                ...
                const storageVerified = downloadResponse.storageVerified !== false; // Default to true if not specified
                const storageErrors = downloadResponse.storageErrors || [];
                
                if (!storageVerified || storageErrors.length > 0) {
                  console.warn(
                    `[Background] ⚠️ URL ${i + 1} data downloaded but storage verification failed. Errors:`,
                    storageErrors,
                  );
                  ...
                  chrome.runtime.sendMessage({
                    ...
                    message: `⚠️ URL ${i + 1} data downloaded but storage verification failed. Retrying...`,
                    alert: true,
                    alertType: "warning",
                  }).catch(() => {});
                  
                  // Retry if storage verification failed
                  if (urlRetryCount < MAX_RETRIES_FOR_URL - 1) {
                    urlRetryCount++;
                    const retryAttempt = urlRetryCount + 1;
                    console.log(
                      `[Background] 🔄 Retrying URL ${i + 1} due to storage verification failure (attempt ${retryAttempt}/${MAX_RETRIES_FOR_URL})...`,
                    );
                    
                    // Close current tab before retry
                    try {
                    ...
                    console.error(
                      `[Background] ❌ URL ${i + 1} storage verification failed after ${MAX_RETRIES_FOR_URL} attempts`,
                    );
                    const failedUrl = {
                      url: url,
                      success: false,
                  ...
                  console.log(
                    `[Background] ✅ API response downloaded and storage verified for URL ${i + 1}`,
                  );
                  processedUrls.push({ url: url, success: true, tabId: currentTab?.id || null, storageVerified: true });
                  urlProcessed = true;
                }
              ...
              continue;
                ...
                const retryAttempt = urlRetryCount + 1; // Display attempt number (1-indexed)
                console.log(
                ...
                chrome.runtime.sendMessage({
                  ...
                  completed: totalProcessed + i,
                  total: Math.max(totalUrlsSeen, totalProcessed + totalUrls),
                  message: `⚠️ URL ${i + 1} is in-flight (no data). Pausing for 5 minutes before retry (attempt ${retryAttempt}/${MAX_RETRIES_FOR_URL})...`,
                  alert: true,
                  alertType: "warning",
                }).catch(() => {});
              ...
              if (urlRetryCount < MAX_RETRIES_FOR_URL - 1) {
                ...
                const retryAttempt = urlRetryCount + 1; // Display attempt number (1-indexed)
                console.log(
                  `[Background] ⏳ Waiting 5 minutes before retry attempt ${retryAttempt}/${MAX_RETRIES_FOR_URL}...`,
                );
                ...
                chrome.runtime.sendMessage({
                  ...
                  completed: totalProcessed + i,
                  total: Math.max(totalUrlsSeen, totalProcessed + totalUrls),
                  message: `⏳ Error on URL ${i + 1}. Waiting 5 minutes before retry attempt ${retryAttempt}/${MAX_RETRIES_FOR_URL}...`,
                  alert: true,
                  alertType: "warning",
                }).catch(() => {});
          ...
          // Update tab reference for closing (only if currentTab still exists)
          tab = currentTab;

          // Wait a bit more to ensure insertion completes (only if tab exists)
          if (tab && tab.id) {
          await new Promise((resolve) => setTimeout(resolve, 500)); // Reduced from 1000ms
          }

          // Close the tab - verify it exists first and wait for closure to complete
          // Note: If all windows were closed due to 200-OK, tab may already be closed
          if (tab && tab.id) {
            try {
          ...
          // Send progress update with accurate counts
          chrome.runtime
            .sendMessage({
              action: "urlProcessingProgress",
              completed: currentIndex,
              total: Math.max(totalUrlsSeen, currentIndex), // Use max to ensure progress doesn't go backwards
          ...
          if (currentIndex >= 40 && currentIndex % 40 === 0) {
            console.log(
              `[Background] 📊 40 URLs processed (Total: ${currentIndex}). Checking API for more URLs...`,
            );
            chrome.runtime
              .sendMessage({
                action: "urlProcessingProgress",
                completed: currentIndex,
                total: Math.max(totalUrlsSeen, currentIndex), // Keep accurate total
                message: `40 URLs processed. Checking API for more URLs...`,
                alert: true,
                alertType: "info",
              })
              .catch(() => {});

            // Update processed URLs count - include URLs processed in current batch up to this point
            allProcessedUrls.push(...processedUrls.slice(0, i + 1));
            totalProcessed += i + 1; // Update to reflect URLs processed so far in this batch
            batchCompletedEarly = true; // Mark that we broke early

            // Break from current batch loop to fetch next batch from API
            break;
          }
      ...
      // Batch complete - add to all processed URLs (only if we didn't break early)
      if (!batchCompletedEarly) {
    ...
    if (failedUrls.length > 0) {
      ...
      const retryResults = await retryFailedUrls(failedUrls, urlsBeforeClear, delayBetweenUrls, totalProcessed);
...
async function setupScheduledExecution() {
  ...
  // Read scheduled hour/minute from config.json, with sane defaults
  let scheduledHour = Number.isFinite(Number(cfg.scheduledHour))
    ? Number(cfg.scheduledHour)
    : 6;
  let scheduledMinute = Number.isFinite(Number(cfg.scheduledMinute))
    ? Number(cfg.scheduledMinute)
    : 58;

  // Clamp to valid ranges
  scheduledHour = Math.min(Math.max(scheduledHour, 0), 23);
  scheduledMinute = Math.min(Math.max(scheduledMinute, 0), 59);
  ...
  const timeString = `${String(scheduledHour).padStart(2, "0")}:${String(
    scheduledMinute,
  ).padStart(2, "0")}`;
  console.log(
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === "closeChromeAfter200OK") {
    // Handle 200-OK case: Close Chrome and clear data after 5 minutes
    // This is a backup in case the service worker was suspended
    try {
      console.log("[Background] ⏰ Alarm fired: Closing Chrome after 5-minute wait for 200-OK (backup handler)...");
      
      // Get stored context
      const stored = await chrome.storage.local.get(["closeChromeContext"]);
      const context = stored.closeChromeContext || {};
      
      // Check if Chrome windows still exist (might have been closed already by main code)
      const windows = await chrome.windows.getAll();
      const hasWindowsToClose = windows.some(w => w.id !== persistentWindowId);
      
      if (hasWindowsToClose) {
        // Close Chrome windows (only if they still exist)
        await closeChromeWindowsSafely();
        ...
        chrome.runtime.sendMessage({
          action: "urlProcessingProgress",
          message: `✅ Chrome closed and data cleared after 5-minute wait (200-OK case - alarm backup)`,
          alert: true,
          alertType: "info",
        }).catch(() => {});
      } else {
        console.log("[Background] ℹ️ Chrome windows already closed, skipping alarm handler");
      }
      
      // Clear stored context
      await chrome.storage.local.remove("closeChromeContext");
      
    } catch (error) {
      console.error("[Background] ❌ Error handling closeChromeAfter200OK alarm:", error);
  ...
  if (alarm.name === "autoRunExtension") {
    ...
    // Read scheduled time for logging
    let scheduledHour = Number.isFinite(Number(cfg.scheduledHour))
      ? Number(cfg.scheduledHour)
      : 6;
    let scheduledMinute = Number.isFinite(Number(cfg.scheduledMinute))
    ...
    scheduledHour = Math.min(Math.max(scheduledHour, 0), 23);
    scheduledMinute = Math.min(Math.max(scheduledMinute, 0), 59);
    const timeStringConfig = `${String(scheduledHour).padStart(
      2,
      "0",
    ...
    // Open extension popup window to show processing status
    try {
      const queryParams = ["standalone=1", "scheduled=1"];
      const popupUrl = chrome.runtime.getURL(
        `popup.html?${queryParams.join("&")}`,
      );
      ...
      persistentWindowId = createdWindow?.id || null;
      console.log("[Background] 📱 Extension popup opened for scheduled processing");
    } catch (error) {
      console.warn("[Background] Could not open extension popup:", error);
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log("[Background] ========================================");
        console.log(
          `[Background] 🏁 Execution completed. Will run again tomorrow at ${timeStringConfig}.`,
        );
      } else {
        ...
        console.log(
          `[Background] Will run again tomorrow at ${timeStringConfig}.`,
        );
      }
    } catch (error) {
      ...
      console.log(
        `[Background] Ready for next scheduled run (tomorrow at ${timeStringConfig}).`,
      );
